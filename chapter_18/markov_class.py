...
def main(script, filename='1871_Dundas_the_little_cap.txt', n=100, order=2):
    try:
        n = int(n)
        order = int(order)
    except ValueError:
        ...
        capMarkov = Markov()
        capMarkov.process_file(filename, order)
        print(capMarkov.suffix_map)
        capMarkov.random_text(n)
        print()
